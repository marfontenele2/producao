  ...
  <div class="no-print">
    <div class="pagina-container">
      <header class="pagina-cabecalho">
        <h1>Impressão Consolidada: Acompanhamento de Saúde Mental</h1>
      </header>
  ...
  <div v-if="dadosRelatorio" id="area-impressao">
    <div class="pagina-a4 borda-impressao">
      <header class="cabecalho-impressao">
        ...
        <h3>RELATÓRIO CONSOLIDADO DE ACOMPANHAMENTO DE SAÚDE MENTAL</h3>
      ...
      <main class="corpo-impressao">
        ...
        >
          <h4 class="titulo-secao-impressao">
            Equipe: {{ equipeData.equipeNome }} (UBS: {{ equipeData.ubsNome }})
          </h4>
          <table class="tabela-impressao">
            ...
            <tbody>
              <tr v-if="equipeData.pacientesComStatus.length === 0">
                <td colspan="5" style="text-align: center">
                  Nenhum paciente na lista ou acompanhamento registrado para esta equipe.
                </td>
              </tr>
              <tr v-for="paciente in equipeData.pacientesComStatus" :key="paciente.id">
...
import { servicoSaudeMental } from '@/modulos/gerente/servicos/servicoSaudeMental'
...
async function gerarRelatorio() {
  ...
  try {
    const promessas = equipesSelecionadas.value.map((equipeId) =>
      servicoSaudeMental.buscarDadosDaEquipe(equipeId).then((dados) => {
        const equipeInfo = listaEquipes.value.find((e) => e.id === equipeId) || {}
        return {
          equipeId,
          equipeNome: equipeInfo.nome || 'Desconhecida',
    ...
    const dadosPorEquipe = resultados
      .map((equipeData) => {
        const acompanhamentosDoMes =
          (equipeData.acompanhamentos && equipeData.acompanhamentos[competencia.value]) || {}
        const pacientesComStatus = (equipeData.pacientes || [])
          .map((paciente) => ({
            ...paciente,
        ...
        return { ...equipeData, pacientesComStatus }
      })
      .filter((equipeData) => equipeData.pacientes.length > 0)

    if (dadosPorEquipe.length === 0) {
      erroBusca.value = `Nenhum dado de Saúde Mental encontrado para as equipes na competência ${competenciaFormatada.value}.`
      return
    }
    dadosRelatorio.value = { dadosPorEquipe }
  } catch (error) {
    console.error('Erro ao consolidar relatório de Saúde Mental:', error)
...
<style scoped>
/* Estilos fieis ao exemplo PaginaImpressaoSCNESAdmin.vue */
@page {
  size: A4 portrait;
  margin: 1.5cm;
}

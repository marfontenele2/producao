  ...
  <div class="no-print">
    <div class="pagina-container">
      <header class="pagina-cabecalho">
        <h1>Impressão: Acompanhamento de Saúde Mental</h1>
      </header>
  ...
  <div v-if="dadosRelatorio" id="area-impressao">
    <div class="pagina-a4 borda-impressao">
      <header class="cabecalho-impressao">
        ...
        <h3>RELATÓRIO DE ACOMPANHAMENTO DE SAÚDE MENTAL</h3>
        <div class="info-cabecalho-grid">
          ...
          <span><strong>UBS:</strong> {{ dadosRelatorio.nomeUbs }}</span>
        ...
        <table class="tabela-impressao">
          <thead>
            <tr>
              ...
              <th>Data de Nasc.</th>
              <th>Sexo</th>
              <th>Acompanhado no CAPS e/ou APS?</th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="dadosRelatorio.pacientesComStatus.length === 0">
              <td colspan="5" style="text-align: center">
                Nenhum paciente na lista ou acompanhamento registrado para esta competência.
              </td>
            </tr>
            <tr v-for="paciente in dadosRelatorio.pacientesComStatus" :key="paciente.id">
              <td class="coluna-nome">{{ paciente.nome }}</td>
              <td>{{ paciente.cns }}</td>
              <td>{{ formatarData(paciente.dataNascimento) }}</td>
              <td>{{ paciente.sexo }}</td>
              <td>{{ paciente.statusAcompanhamento }}</td>
            </tr>
          </tbody>
        </table>
      </main>
...
const nomeGerenteAssinatura = ref(storeUsuario.usuario?.nome || '')

const listaEquipes = ref([])
const carregandoEquipes = ref(true)
const equipeIdSelecionada = ref(null)
...
async function gerarRelatorio() {
  ...
  try {
    const equipeId = equipeIdSelecionada.value
    if (!equipeId) throw new Error('Por favor, selecione uma equipe.')

    const dados = await servicoSaudeMental.buscarDadosDaEquipe(equipeId)

    if (dados && dados.pacientes.length > 0) {
      // ===================================================================
      // === CORREÇÃO DO ERRO: Acesso seguro à propriedade de acompanhamentos
      // ===================================================================
      const acompanhamentosDoMes =
        (dados.acompanhamentos && dados.acompanhamentos[competencia.value]) || {}
      // ===================================================================

      const pacientesComStatus = dados.pacientes
        .map((paciente) => ({
          ...paciente,
          statusAcompanhamento: acompanhamentosDoMes[paciente.id] || 'Não Informado',
        }))
        .sort((a, b) => a.nome.localeCompare(b.nome))

      const equipeInfo = listaEquipes.value.find((e) => e.id === equipeId)

      dadosRelatorio.value = {
        pacientesComStatus,
        nomeEquipe: equipeInfo?.nome || 'N/A',
        nomeUbs: equipeInfo?.nomeUbs || storeUsuario.usuario?.nomeUbs || 'N/A',
      }
    } else {
      erroBusca.value = `Nenhum paciente cadastrado para a equipe selecionada.`
    }
  } catch (error) {
    console.error('Erro ao gerar relatório de Saúde Mental:', error)
    erroBusca.value = `Ocorreu um erro ao buscar os dados do relatório: ${error.message}`
  } finally {
